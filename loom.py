...
def get_line(nail1: tuple[int, int], nail2: tuple[int, int]) -> list[tuple[int, int]]:
    """
...
def find_darkest_line(image: np.array(list[list[float]]), possible_strands: list[Strand]) -> Strand:
    ...
    print(f"found strand with mean of {mean_values[min_index]}")
...
class Loom(object):
    ...
    def weave(self, bound, intensity=25):
        counter = 0
        current_nail = self.nails[0]
        while counter < bound:
            current_strand = find_darkest_line(self.image, self.strands[current_nail])
            for index in current_strand.line:
                self.image[index] = min(WHITE, self.image[index] + intensity)
                self.canvas[index] = max(BLACK, self.canvas[index] - intensity)
            counter += 1
